...
""" Multi-scale variance stabilising transform
...
def dec1D(data, J=None,hj=False):
    import numpy as np
    if J is None:
        J = int(np.floor(np.log2(data.size - 1)))

    if len(kernels) != J:
        make_kernels(J)
        assert(len(kernels) == J)
    """if len(hjs) < J or hjs[0].shape[0] != data.size:
        make_hjs(data.shape[0])
        assert(len(hjs) == J)"""
    ...
    ajs = list()
    for j in range(J):
        tmp = convolve(ajm1,kernels[j], mode='constant')
        ajs.append(tmp.copy())
        djs.append(ajm1 - tmp)
        ajm1 = tmp
...
def ascombe(img):
    return 2*np.sign(img + 3/8)*np.sqrt(np.abs(img + 3/8))

def iascombe(y):
    return np.sign(y/2)*(y/2)**2-3/8

def test_ascombe():
    x=np.linspace(-10,10,20)
    for a in x:
        print(ascombe(iascombe(a)),a)

def ascombe_denoise(img):
    imas=ascombe(img)
    imas[np.abs(imas-np.mean(imas))<3*np.std(imas)]=0
    return iascombe(imas)

def high_poisson_denoising(im,fac=3):
    ajs,djs = dec(im)
    dj_den=list()
    for d in djs:
        den = ascombe(d)
        print("sigma den:",np.std(den))
        den[np.abs(den)<fac*np.std(den)]=0
        dj_den.append(iascombe(den))
    return ajs,dj_den
